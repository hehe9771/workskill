...
2. 创建输出目录并下载视频（自动获取视频标题作为文件名）：
   ```bash
   mkdir -p output
   bash ~/.claude/skills/videobook/scripts/data-processing/download_video.sh "<video_url>" "output"
   ```
...
1. 运行提取脚本（使用步骤1获取的视频名称）：
   ```bash
   bash ~/.claude/skills/videobook/scripts/data-processing/extract_audio.sh "output/{视频名称}.mp4" "output/{视频名称}_transcript.txt"
   ```
